...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!projectId || !userId) {
      return new Response(JSON.stringify({ error: 'Missing required context keys (Project or User ID).' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json', ...corsHeaders }
      });
    }
    ...
    return new Response(JSON.stringify({ success: true, message: 'Task created successfully!' }), {
      status: 201,
      headers: { 'Content-Type': 'application/json', ...corsHeaders }
    });
    ...
    return new Response(JSON.stringify({ error: 'Erro ao criar tarefa no banco de dados.', details: error.message }), {
      status: 500,
      headers: { 'Content-Type': 'application/json', ...corsHeaders }
    });
  }
