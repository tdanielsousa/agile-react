...
export async function onRequest(context) {
  ...
  try {
    ...
    // 1. Validation checks
    if (!projectId || !status) {
      return new Response(JSON.stringify({ error: 'Missing required fields: projectId and status.' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json', ...corsHeaders }
      });
    }
    ...
    if (normalizedStatus !== 'ACTIVE' && normalizedStatus !== 'OVER') {
      return new Response(JSON.stringify({ error: 'Invalid status. Must be either "ACTIVE" or "OVER".' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json', ...corsHeaders }
      });
    }
    ...
    return new Response(JSON.stringify({ error: 'Erro ao atualizar o estado do projeto.', details: error.message }), {
      status: 500,
      headers: { 'Content-Type': 'application/json', ...corsHeaders }
    });
  }
